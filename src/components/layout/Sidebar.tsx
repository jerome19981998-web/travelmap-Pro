...
import { Map, BarChart2, Heart, Share2, Settings, LogOut, Trophy, ChevronRight } from "lucide-react";
            ...
            return (
              <li key={href}>
                <Link href={href} className={clsx(
                  ...
                  active ? "bg-emerald-500/15 text-emerald-400 border border-emerald-500/20" : "text-[var(--text-secondary)] hover:bg-white/5 hover:text-[var(--text-primary)]"
                )}>
        ...
        <div className="flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-white/5 transition-colors">
          <div className="w-7 h-7 flex-shrink-0 rounded-full bg-gradient-to-br from-emerald-400 to-teal-600 flex items-center justify-center overflow-hidden">
            {profile?.avatar_url ? (
              ...
              <span className="text-xs text-white font-bold">{(profile?.full_name || user.email || "U")[0].toUpperCase()}</span>
            )}
          </div>
          <div className="hidden lg:flex flex-col flex-1 min-w-0">
            <span className="text-xs font-medium text-[var(--text-primary)] truncate">{profile?.full_name || "Traveler"}</span>
            <span className="text-xs text-[var(--text-muted)] truncate">{user.email}</span>
          </div>
          <button onClick={handleSignOut} className="hidden lg:block text-[var(--text-muted)] hover:text-red-400 transition-colors p-1" title={t.signOut}>
