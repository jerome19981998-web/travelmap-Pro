...
import {
  Map, BarChart2, Heart, Share2, Settings, LogOut,
  Globe, Trophy, ChevronRight
} from "lucide-react";
import Image from "next/image";
import { clsx } from "clsx";

const nav = [
  { href: "/dashboard", icon: Map, label: "Map", exact: true },
  { href: "/dashboard/stats", icon: BarChart2, label: "Stats" },
  { href: "/dashboard/wishlist", icon: Heart, label: "Wishlist" },
  { href: "/dashboard/badges", icon: Trophy, label: "Badges" },
  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 z-40 flex flex-col glass-elevated border-r border-[var(--surface-border)] w-16 lg:w-64 transition-all duration-300">
      ...
      <div className="flex items-center gap-3 px-4 py-5 border-b border-[var(--surface-border)]">
        <div className="w-8 h-8 flex-shrink-0 rounded-xl bg-gradient-to-br from-emerald-400 to-teal-600 flex items-center justify-center text-sm shadow-lg shadow-emerald-500/20">
          🗺️
        </div>
        <span className="hidden lg:block font-bold text-sm text-[var(--text-primary)] tracking-tight">
          TravelMap Pro
        </span>
      </div>
            ...
            return (
              <li key={href}>
                ...
                >
                  ...
                  {active && (
                    <ChevronRight className="hidden lg:block w-3 h-3 ml-auto text-emerald-400/50" />
                  )}
                </Link>
              </li>
            );
