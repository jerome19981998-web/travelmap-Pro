  ...
  const renderMarkers = useCallback(() => {
    ...
    if (effectiveFilter !== "wishlist") {
      visits.forEach(visit => {
        ...
        const placeName = escapeHtml(visit.place_name);
        const countryName = escapeHtml(visit.country_name);
        marker.bindTooltip(
          `<div style="min-width:130px">
            <div style="font-weight:700;font-size:13px;margin-bottom:3px">${placeName}</div>
            ${countryName ? `<div style="font-size:11px;opacity:0.65">${countryName}</div>` : ""}
    ...
    if (effectiveFilter === "countries" || effectiveFilter === "cities" || effectiveFilter === "neighborhoods" || effectiveFilter === "wishlist") {
      ...
      if (showWishlist || effectiveFilter !== "countries") {
        wishlist.forEach(item => {
          ...
          marker.bindTooltip(
            `<div style="min-width:120px"><div style="font-weight:700;font-size:13px">💜 ${placeName}</div>${countryName ? `<div style="font-size:11px;opacity:0.65">${countryName}</div>` : ""}</div>`,
            { className: "map-tooltip", direction: "top", offset: [0, -10] }
          );
          markersLayerRef.current!.addLayer(marker);
        });
